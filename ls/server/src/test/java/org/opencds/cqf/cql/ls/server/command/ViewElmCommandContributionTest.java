...
class ViewElmCommandContributionTest {
    ...
    void getCommands() {
        ...
        assertEquals("org.opencds.cqf.cql.ls.viewElm",
    ...
    void executeCommand() {
        ...
        params.setArguments(Collections.singletonList(JsonParser
                .parseString("\"\\/org\\/opencds\\/cqf\\/cql\\/ls\\/server\\/One.cql\"")));
        viewElmCommandContribution.executeCommand(params).thenAccept(result -> {
            try {
                String expectedXml =
                        new String(Files.readAllBytes(Paths.get("src/test/resources/One.xml")))
                                .trim().replaceAll("\\s+", "");
